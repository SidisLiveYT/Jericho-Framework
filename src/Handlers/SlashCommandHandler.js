...
export class SlashCommandHandler {
    ...
    constructor(client, CreateOptions) {
        this.client = client;
        this.guild = CreateOptions.guild ? GuildResolver(client, CreateOptions.guild, {
            ifmessage: true
        }) : null;
    ...
    async set(commands) {
        ...
        if (!this.SlashCommands && this.deployed) throw SyntaxError(`Slash Command has been already Deployed with Previous Set Values | Try Setting New Slash Commands`);
        else if (!this.SlashCommands) throw SyntaxError(`Slash Command has not been Set/Saved | Try Setting Slash Commands - <SlashCommandHandler>.set()`);
        else if (!this.client.application?.owner) await this.client.application?.fetch();
        const SlashCommandInstance = new SlashCommandBuilder(Client, this.SlashCommands);
    ...
    async deploy() {
        if (this.deployed || !this.SlashCommands) throw SyntaxError(`No New Slash Command has been Set to Deploy | Try Setting New Slash Commands- <SlashCommandHandler>.set()`);
        else if (this.ApplicationCommands.length <= 0) throw SyntaxError(`No Slash Command has been Set to Deploy | try - <SlashCommandHandler>.set()`);
        else if (!this.client.application?.owner) await this.client.application?.fetch();
        return await this.client.application.commands.set(this.SlashCommands)
    ...
    async get(CommandId) {
        ...
        if (CommandId) {
            return this.client.application.commands.fetch(`${CommandId}`)
    ...
    #DeleteApplciationCommands(CommandId) {
        ...
        for (count = 0; count < this.ApplicationCommands.length; ++count) {
            ...
            this.client.application.commands.delete(`${this.ApplicationCommands[count].id}`).then(() => {
                    this.ApplicationCommands[count].splice(count, 1);
                })
                .catch(error => {
                    throw Error(error);
