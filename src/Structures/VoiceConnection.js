...
export class VoiceConnectionBuilder {
	...
	disconnect() {
		...
		if (!SuccessBooleanResult) throw TypeError(`Voice Connection can't be disconnected!`);
	...
	destroy(AdapterAvailable = true) {
		if (!this.VoiceConnection) throw TypeError(`No Voice Connection found in Handler!`);
		const SuccessBooleanResult = this.VoiceConnection.destroy({ adapterAvailable: AdapterAvailable });
		if (!SuccessBooleanResult) throw TypeError(`Voice Connection can't be distroyed!`);
		else {
